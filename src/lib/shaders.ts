...
// Polar seam fix: equirectangular vUv.x (longitude) converges to a single
// point at each pole, so the screen-space derivatives of vUv that
// perturbNormal2Arb relies on become erratic there, perturbing N in wildly
// different directions between neighboring fragments -- a radial starburst.
// POLE_FADE_START is how far toward the pole (in vUv.y, 0-1) the normal-map
// relief starts fading out, letting N fall back to the smooth geometric
// surface normal right at the pole.
...
void main() {
  ...
  mapN.xy *= 0.5 * (1.0 - poleDist); // subtle relief, faded out near the poles
